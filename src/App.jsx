...
// EXCEPTION: Allow copy/paste in text inputs only
...
user-select: none;
    ...
    {view === 'home' && (
      <div className="w-full max-w-[580px] mx-auto px-4 z-10 relative text-center animate-in fade-in duration-300">
        ...
        <main className="space-y-10 pb-20 text-left">
          ...
          <div className="lighthouse-neon-wrapper shadow-3xl mx-2 sm:mx-0">
            <div className="lighthouse-neon-content p-8 sm:p-14 text-left space-y-10">
              ...
              <div className="space-y-4">
                 ...
                 <input type="tel" value={genTo} onChange={e => setGenTo(e.target.value)} className="input-premium font-sans text-base sm:text-lg" placeholder="+1 999 999 9999" />
              </div>
              <div className="space-y-4">
                 ...
                 <input type="text" value={companyName} onChange={e => setCompanyName(e.target.value)} className="input-premium font-sans text-base sm:text-lg" placeholder="Your Organization Name" />
              </div>
              <div className="space-y-4">
                 ...
                 <div className="relative">
                    <textarea value={genMsg} onChange={e => setGenMsg(e.target.value)} rows="4" className={`input-premium w-full text-base sm:text-lg font-sans leading-relaxed ${isGenMsgForbidden ? '!text-[#FE2C55] !border-[#FE2C55] shadow-[0_0_15px_rgba(254,44,85,0.3)]' : ''}`} placeholder="Draft your pre-defined payload here to receive via SMS 📲" />
              ...
              {showInstructions && (
                <div className="p-6 sm:p-8 bg-white/[0.03] border border-[#25F4EE]/20 rounded-2xl animate-in slide-in-from-top-2">
                   <h5 className="text-[11px] sm:text-[12px] text-[#25F4EE] mb-4 text-center">STRATEGIC DEPLOYMENT GUIDE:</h5>
                   <ul className="text-[10px] sm:text-[11px] text-white/50 space-y-4 leading-relaxed text-left">
          ...
          {generatedLink && (
            <div className="animate-in zoom-in-95 duration-300 space-y-6 px-2 sm:px-0">
              <div className="bg-[#0a0a0a] border border-[#25F4EE]/20 rounded-[2.5rem] p-8 sm:p-12 text-center shadow-2xl">
                ...
                <input readOnly value={generatedLink} onClick={e=>e.target.select()} className="w-full bg-black/40 border border-white/5 rounded-xl p-5 text-xs sm:text-sm text-[#25F4EE] font-mono text-center outline-none mb-8 border-dashed font-medium !text-transform-none truncate" />
          ...
          <div className="pt-24 pb-16 text-left px-4 sm:px-0">
             <div className="flex items-center gap-4 mb-12 sm:mb-14"><HelpCircle size={32} className="text-[#FE2C55]"/><h3 className="text-3xl sm:text-4xl text-white tracking-tight">PROTOCOL FAQ</h3></div>
        ...
        {isMaster && (
            <div className="bg-[#0a0a0a] border border-amber-500/30 p-8 sm:p-10 rounded-3xl sm:rounded-[2.5rem] shadow-[0_0_30px_rgba(245,158,11,0.1)] flex flex-col relative overflow-hidden mb-10">
              ...
              <form onSubmit={handleBroadcastPush} className="flex gap-4 flex-col sm:flex-row items-stretch sm:items-center">
                <input type="text" value={broadcastMsg} onChange={e=>setBroadcastMsg(e.target.value)} placeholder="Enter push notification for all users..." className="input-premium flex-1 font-sans text-base sm:text-lg !text-transform-none" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 sm:gap-10 mb-20">
            ...
            <div className="bg-[#0a0a0a] border border-white/10 p-8 sm:p-10 rounded-3xl sm:rounded-[2.5rem] shadow-2xl flex flex-col relative overflow-hidden flex-1">
              ...
              <form onSubmit={handleQuickSend} className="space-y-5 sm:space-y-6 flex flex-col flex-1">
                <div>
                  ...
                  <input type="tel" value={genTo} onChange={e=>setGenTo(e.target.value)} placeholder="+1 000 000 0000" className="input-premium text-base sm:text-lg font-sans !text-transform-none" />
                </div>
                <div className="flex-1 flex flex-col">
                  ...
                  <textarea rows="4" value={genMsg} onChange={e=>setGenMsg(e.target.value)} placeholder="Draft your SMS here..." className="input-premium flex-1 text-base sm:text-lg font-sans leading-relaxed !text-transform-none resize-none"></textarea>
                </div>
          ...
          <div className="lg:col-span-2 bg-[#0a0a0a] rounded-3xl sm:rounded-[2.5rem] border border-white/10 shadow-3xl overflow-hidden flex flex-col h-full min-h-[500px] sm:min-h-[600px]">
             ...
             <div className="flex-1 overflow-x-auto custom-scrollbar bg-black/40">
               {isMaster ? (
                 <table className="w-full text-left font-sans font-medium !text-transform-none min-w-[700px]">
                     ...
                     {subscribersList.map(sub => (
                        <React.Fragment key={sub.id}>
                            <tr onClick={() => setExpandedAdminRow(expandedAdminRow === sub.id ? null : sub.id)} className={`hover:bg-white/[0.02] transition-colors cursor-pointer group ${expandedAdminRow === sub.id ? 'bg-white/[0.03]' : ''}`}>
                               <td className="px-8 sm:px-10 py-6 sm:py-8">
                                  <div className="flex items-center gap-4">
                                      ...
                                      <div>
                                          ...
                                          <p className="text-[10px] sm:text-[11px] text-white/40 tracking-widest font-mono mt-1.5">{sub.email} | {sub.tier}</p>
                                      </div>
                                  </div>
                               </td>
                               <td className="px-8 sm:px-10 py-6 sm:py-8 text-center text-sm sm:text-base text-white font-black">
                               ...
                               <td className="px-8 sm:px-10 py-6 sm:py-8 flex justify-end gap-2 sm:gap-3 mt-2 sm:mt-3">
                                    ...
                                    <td colSpan="3" className="p-0 border-b border-white/5">
                                        <div className={`bg-black border-l-4 p-8 animate-in slide-in-from-top-2 ${sub.id === 'AI_SMART_CHAT' ? 'border-amber-500' : 'border-[#25F4EE]'}`}>
                                            {sub.leads.length > 0 ? (
                                                   ...
                                                   <tbody className="divide-y divide-white/5">
                                                       {sub.leads.map(l => (
                                                          <tr key={l.id} className="hover:bg-white/5 group">
                                                             ...
                                                             <td className="py-4 px-5 text-xs sm:text-sm text-white">{l.nome_cliente} {sub.id === 'AI_SMART_CHAT' && <span className="ml-3 text-[9px] tracking-widest text-black bg-amber-500 px-2 py-0.5 rounded-sm">NEXUS AGENT</span>}</td>
                                                             <td className="py-4 px-5 text-xs sm:text-sm">
                                                                <select
                                                                  ...
                                                                  className="bg-[#111] border border-white/10 text-white/50 text-[10px] sm:text-[11px] px-3 py-1.5 rounded-lg outline-none font-black tracking-widest appearance-none cursor-pointer hover:border-[#25F4EE]/30 transition-colors"
                                                                >
                                                                  {folders.map(f => (
                                                ...
                                                <p className="text-xs sm:text-sm text-white/30 tracking-widest text-center py-8 font-black">NO LEADS CAPTURED BY THIS GATEWAY YET.</p>
                                            )}
                                        </div>
                                    </td>
                                </tr>
                 ...
                 <>
                   {logs.length > 0 ? (
                     ...
                     <div className="flex flex-col items-center justify-center h-full p-12 sm:p-24 opacity-20 text-center"><Lock size={56} className="mb-5 text-white" /><p className="text-[11px] sm:text-xs tracking-widest font-black">VAULT STANDBY</p><p className="text-[9px] sm:text-[10px] mt-3 font-sans font-medium !text-transform-none">NO ACTIVE INTERCEPTIONS.</p></div>
                   )}
                 </>
               )}
             </div>
             {!isPro && !isMaster && logs.length > 0 && (
               <div className="p-8 sm:p-10 bg-gradient-to-t from-[#FE2C55]/10 to-transparent border-t border-[#FE2C55]/20 flex flex-col sm:flex-row items-center justify-between gap-5">
                    ...
                    {stagedQueue.map((task, idx) => (
                       <div key={task.id || idx} className={`bg-[#111] border border-white/5 rounded-2xl p-5 sm:p-6 transition-colors flex flex-col h-[160px] sm:h-[180px] ${isDispatching && idx === 0 ? 'border-[#25F4EE] shadow-[0_0_15px_rgba(37,244,238,0.3)] animate-pulse' : 'hover:border-[#25F4EE]/30 group'}`}>
                          ...
                          <textarea 
                            ...
                            className="w-full flex-1 bg-black/50 border border-white/5 rounded-xl p-3.5 text-xs sm:text-sm text-white/80 resize-none font-sans !text-transform-none leading-relaxed focus:border-[#25F4EE]/50 outline-none disabled:opacity-50 custom-scrollbar" 
                          />
                       </div>
                    ))}
                 </div>
                  ...
                  <div className="bg-[#111] border border-white/5 rounded-2xl sm:rounded-[2rem] flex flex-col items-center justify-center p-8 sm:p-10 min-h-[220px] sm:min-h-[250px] text-center shadow-inner relative overflow-hidden mt-5 lg:mt-0">
                    ...
                    {!smsQueueCount && (
                      <div className="absolute bottom-5 left-5 right-5 p-4 sm:p-5 bg-white/[0.02] border border-white/5 rounded-2xl text-left hidden sm:block">
                         ...
                         <p className="text-[9px] sm:text-[10px] text-white/30 font-sans !text-transform-none leading-relaxed">Secure background routing active. To preserve operational stealth, transmissions operate independently and will not be visible in your device's native SMS outbox.</p>
                      </div>
                    )}
                  </div>
               </div>
            ...
            {linksHistory.length > 0 ? linksHistory.map(l => (
              <div key={l.id} className="p-6 sm:p-10 border-b border-white/5 flex flex-col md:flex-row justify-between md:items-center gap-5 sm:gap-8 hover:bg-white/[0.02] transition-colors">
                <div className="flex-1 overflow-hidden">
                   ...
                   <p className="text-base sm:text-lg text-[#25F4EE] truncate font-sans !text-transform-none max-w-[280px] sm:max-w-[600px]">{l.url}</p>
                   <p className="text-[10px] sm:text-[11px] text-white/40 mt-2.5 leading-relaxed font-sans !text-transform-none truncate">HOST: {String(l.company)} | PAYLOAD: {String(l.msg).substring(0,80)}...</p>
                </div>
        ...
        <div className="lighthouse-neon-wrapper w-full max-w-lg shadow-3xl">
          <div className="lighthouse-neon-content p-10 sm:p-14 md:p-20 relative">
            ...
            <form onSubmit={handleAuthSubmit} className="space-y-6 sm:space-y-8 text-left">
              {!isLoginMode && (<><input required placeholder="FULL LEGAL NAME" value={fullNameInput} onChange={e=>setFullNameInput(e.target.value)} className="input-premium text-sm sm:text-base w-full font-sans font-medium !text-transform-none" /><input required placeholder="NICKNAME" value={nicknameInput} onChange={e=>setNicknameInput(e.target.value)} className="input-premium text-sm sm:text-base w-full font-sans font-medium !text-transform-none" /><input required type="tel" placeholder="+1 999 999 9999" value={phoneInput} onChange={e=>setPhoneInput(e.target.value)} className="input-premium text-sm sm:text-base w-full font-sans font-medium !text-transform-none" /></>)}
              <input required type="email" placeholder="EMAIL IDENTITY..." value={email} onChange={e=>setEmail(e.target.value)} className="input-premium text-sm sm:text-base w-full font-sans font-medium !text-transform-none" />
              <div className="relative">
                <input required type={showPass ? "text" : "password"} placeholder="SECURITY KEY..." value={password} onChange={e=>setPassword(e.target.value)} className="input-premium text-sm sm:text-base w-full font-sans font-medium !text-transform-none pr-14" />
              ...
              <button type="submit" disabled={loading} className="btn-strategic !bg-[#25F4EE] !text-black text-[12px] sm:text-sm mt-5 shadow-xl w-full tracking-widest py-5 sm:py-6 font-black">{loading ? 'VERIFYING GATEWAY...' : (isWelcomeTrial ? 'UNLOCK MY 60 TRANSMISSIONS' : 'AUTHORIZE ACCESS')}</button>
              <button type="button" onClick={() => { setIsLoginMode(!isLoginMode); }} className="w-full text-[11px] sm:text-[12px] text-white/30 hover:text-white tracking-[0.2em] sm:tracking-[0.4em] mt-10 sm:mt-12 text-center transition-all px-2 font-black">{isLoginMode ? "CREATE NEW OPERATOR? REGISTER" : "ALREADY A MEMBER? LOGIN"}</button>
            </form>
          </div>
        </div>
      </div>
  ...
  <footer className="mt-auto pb-16 sm:pb-24 w-full z-[100] px-8 sm:px-12 border-t border-white/5 pt-16 sm:pt-24 text-left bg-[#010101] relative">
    <div className="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-10 sm:gap-14 text-[11px] sm:text-[12px] tracking-[0.15em] sm:tracking-widest text-white/30 font-black">
    ...
    <div className="max-w-7xl mx-auto mt-16 sm:mt-20 pt-10 border-t border-white/5 flex justify-center">
       <p className="text-[10px] sm:text-[12px] text-white/20 tracking-[0.3em] sm:tracking-[0.5em] text-center w-full px-5 text-glow-white font-black">© 2026 CLICKMORE DIGITAL | EXCLUSIVE SECURITY PROTOCOL</p>
    </div>
  </footer>
      ...
      <div className="bg-[#0a0a0a] border border-[#25F4EE]/30 w-full max-w-2xl rounded-[2rem] sm:rounded-[2.5rem] shadow-[0_0_50px_rgba(37,244,238,0.15)] flex flex-col max-h-[85vh] overflow-hidden relative">
         ...
         <div className="p-8 sm:p-10 border-b border-white/10 flex justify-between items-center bg-[#111] shrink-0">
            <div className="flex items-center gap-4 sm:gap-5">
               ...
               <h3 className="text-2xl sm:text-3xl text-white tracking-tight font-black">{legalContent === 'ZERO' ? 'ZERO TOLERANCE POLICY' : 'LEGAL PROTOCOL'}</h3>
            </div>
         ...
         <div className="p-8 sm:p-12 overflow-y-auto custom-scrollbar flex-1 bg-gradient-to-b from-[#111] to-black">
            {legalContent === 'ZERO' ? (
              <p className="text-base sm:text-lg text-white/70 font-sans !text-transform-none leading-loose sm:leading-loose">
                <span className="text-amber-500 font-bold block mb-5">STRICT COMPLIANCE ENFORCEMENT</span>
            ...
            <div className="mt-10 sm:mt-14 p-6 sm:p-8 bg-[#25F4EE]/5 border border-[#25F4EE]/20 rounded-2xl sm:rounded-3xl">
               ...
               <p className="text-sm sm:text-base text-white/40 font-sans !text-transform-none leading-relaxed">By engaging with the SMART SMS PRO ecosystem, your footprint is subjected to AES-256 standard cryptographic masking. No unauthorized external relays possess decryption keyframes.</p>
            </div>
         </div>

         <div className="p-8 sm:p-10 border-t border-white/10 bg-black shrink-0 flex justify-end">
            <button onClick={() => setLegalContent(null)} className="w-full sm:w-auto px-10 sm:px-12 py-4 sm:py-5 bg-[#25F4EE] text-black text-[12px] sm:text-sm font-black tracking-widest rounded-xl hover:scale-[1.02] transition-transform shadow-[0_0_20px_rgba(37,244,238,0.3)]">ACKNOWLEDGE & CLOSE</button>
         </div>
      </div>
    </div>
  )}
    ...
    <div className="fixed inset-0 z-[600] bg-black/95 backdrop-blur-xl flex items-center justify-center p-4 sm:p-6 text-center animate-in fade-in">
      <div className="bg-[#0a0a0a] border border-[#25F4EE]/50 rounded-3xl sm:rounded-[2rem] p-8 sm:p-10 max-w-lg w-full relative shadow-[0_0_50px_rgba(37,244,238,0.2)] max-h-[90vh] overflow-y-auto custom-scrollbar flex flex-col items-center">
        ...
        <div className="flex justify-center mb-6 sm:mb-8 mt-5">
          <div className="p-5 bg-[#25F4EE]/10 rounded-full border border-[#25F4EE]/30 animate-pulse">
            <DownloadCloud size={36} className="text-[#25F4EE]" />
          </div>
        </div>
