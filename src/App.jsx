...
// --- MASTER ADMIN ACCESS ---
const ADMIN_MASTER_ID = "YGepVHHMYaN9sC3jFmTyry0mYZO2"; // <--- INSIRA SEU UID REAL AQUI

// --- FAQ COMPONENT (FIXED - NO UNDEFINED) ---
const FAQItem = ({ q, a }) => {
...
export default function App() {
  ...
  // --- 2. IDENTITY BOOTSTRAP (MASTER PRIORITY) ---
  useEffect(() => {
  ...
  const handleProtocolHandshake = async () => {
    ...
    if(!captureForm.phone.startsWith('+')) return alert("Mobile identification must start with '+' (ex: +1 999 999 9999)");
    ...
    try {
      ...
      if (ownerProfSnap.exists()) {
         ...
         if (op.tier !== 'MASTER') await updateDoc(pubRef, { smsCredits: increment(-1) });
      }
      ...
      {view === 'capture' ? (
        ...
        <>
          ...
          <div className="pt-28 flex-1 pb-10 relative">
            ...
            {view === 'home' && (
              <div className="w-full max-w-[540px] mx-auto px-4 z-10 relative text-center font-black italic animate-in fade-in duration-500 leading-none">
                <header className="mb-14 text-center flex flex-col items-center leading-none">
                  <div className="lighthouse-neon-wrapper mb-4 font-black italic"><div className="lighthouse-neon-content px-10 py-4 leading-none"><h1 className="text-3xl font-black italic uppercase text-white text-glow-white leading-none uppercase">SMART SMS PRO</h1></div></div>
                ...
                <main className="space-y-8 pb-20 text-left font-black italic leading-none">
                  {user && (
                    <div className="flex justify-center mb-2 animate-in fade-in zoom-in duration-500 font-black italic">
                      <button onClick={() => setView('dashboard')} className="btn-strategic !bg-[#25F4EE] !text-black text-xs w-full max-w-[420px] group italic font-black uppercase py-6 shadow-[0_0_30px_#25F4EE] font-black italic leading-none"><LayoutDashboard size={24} /> ACCESS OPERATOR HUB</button>
                    </div>
                  )}

                  <div className="lighthouse-neon-wrapper shadow-3xl font-black italic leading-none">
                    <div className="lighthouse-neon-content p-8 sm:p-12 text-left space-y-8 leading-none font-black italic">
                      ...
                      <div className="space-y-3 leading-none font-black italic">
                         ...
                         <div className="relative font-black italic leading-none">
                            ...
                            <button onClick={()=>setShowInstructions(!showInstructions)} className="absolute right-3 bottom-3 p-1.5 bg-[#25F4EE]/10 rounded-lg text-[#25F4EE] hover:bg-[#25F4EE]/20 transition-all font-black italic leading-none"><HelpCircle size={14}/></button>
                         </div>
                      </div>
                      {showInstructions && (
                        <div className="p-6 bg-white/[0.03] border border-[#25F4EE]/20 rounded-2xl animate-in slide-in-from-top-2 font-black italic leading-none">
                      ...
                      <button onClick={handleGenerate} className="btn-strategic !bg-[#25F4EE] !text-black text-xs italic font-black uppercase py-5 w-full shadow-2xl leading-none font-black italic uppercase font-black italic">Generate Smart Link <ChevronRight size={18} /></button>
                    </div>
                  </div>
                  ...
                  {!user && (
                    <div className="flex flex-col items-center gap-6 mt-8 w-full animate-in zoom-in-95 duration-500 pb-10 leading-none font-black italic text-center uppercase font-black italic leading-none">
                      <button onClick={() => {setIsLoginMode(false); setView('auth')}} className="btn-strategic !bg-white !text-black text-xs w-full max-w-[420px] group italic font-black uppercase py-6 leading-none shadow-xl uppercase font-black italic"><Rocket size={24} className="group-hover:animate-bounce" /> Start 60 Free Handshakes</button>
                      <button onClick={() => window.open("https://buy.stripe.com/nexus_access", '_blank')} className="btn-strategic !bg-[#25F4EE] !text-black text-xs w-full max-w-[420px] group italic font-black uppercase py-6 leading-none shadow-[0_0_20px_#25F4EE] uppercase font-black italic"><Star size={24} className="animate-pulse" /> Upgrade to Elite Member</button>
                    </div>
                  )}

                  <div className="pt-20 pb-12 text-left font-black italic leading-none uppercase font-black italic leading-none">
                     <div className="flex items-center gap-3 mb-12 text-left leading-none uppercase font-black italic font-black italic leading-none"><HelpCircle size={28} className="text-[#FE2C55] font-black italic leading-none font-black italic"/><h3 className="text-3xl font-black uppercase text-white tracking-widest leading-none font-black italic uppercase font-black italic">Protocol FAQ</h3></div>
                     <div className="space-y-2 text-left font-black italic leading-tight uppercase font-black italic font-black italic leading-none">
                        <FAQItem q="Why utilize our exclusive protocol instead of standard redirects?" a="Standard market redirects often trigger automated heuristics instantly. Our proprietary protocol dynamically formats headers to mirror organic signatures, significantly enhancing final delivery rates." />
                        <FAQItem q="Is the cryptographic vault fully impenetrable and compliant?" a="Absolutely. Operating under a robust Zero-Knowledge architecture, lead metadata remains exclusively encrypted within your session context." />
                        <FAQItem q="How does the system ensure long-term standing protection?" a="Our ecosystem employs an intelligent pacing engine that meticulously manages dispatch intervals, ensuring sustainable high-volume operations." />
                        <FAQItem q="What is the strategic advantage of Advanced AI Synthesis?" a="The IA Agent dynamically optimizes payload contexts in real-time. By utilizing advanced frameworks, it ensures each dispatch maintains a unique organic fingerprint." />
                     </div>
                  </div>
                </main>
              </div>
            ...
            {view === 'dashboard' && (
              <div className="w-full max-w-7xl mx-auto py-10 px-6 font-black italic text-left leading-none uppercase animate-in fade-in duration-700 font-black italic leading-none font-black italic">
                <div className="flex flex-col lg:flex-row justify-between lg:items-end gap-6 mb-12 text-left font-black italic leading-none font-black italic font-black italic leading-none">
                  ...
                  <div className="flex items-center gap-4 flex-wrap leading-none font-black italic uppercase text-center font-black italic font-black italic leading-none font-black italic">
                     <button onClick={() => setView('home')} className="flex items-center gap-2 bg-white/5 border border-white/10 px-6 py-4 rounded-xl hover:bg-white/10 transition-colors text-[10px] font-black italic uppercase text-white leading-none font-black italic font-black italic leading-none font-black italic"><Zap size={14} className="text-[#25F4EE]"/> LINK GENERATOR</button>
                     <div className="bg-[#0a0a0a] border border-white/10 px-6 py-3 rounded-[1.5rem] text-center shadow-3xl leading-none uppercase font-black font-black italic font-black italic leading-none font-black italic">
                        <p className="text-[8px] font-black text-white/30 uppercase mb-2 italic tracking-widest leading-none font-black uppercase font-black italic font-black italic leading-none font-black italic">Nodes</p>
                     ...
                     <div className="bg-[#0a0a0a] border border-white/10 px-6 py-3 rounded-[1.5rem] text-center shadow-3xl border-b-2 border-b-[#25F4EE] text-center leading-none uppercase font-black font-black italic font-black italic leading-none font-black italic">
                ...
                <div className="space-y-10 font-black italic uppercase font-black italic leading-none">
                   <div className={`bg-[#0a0a0a] border border-white/10 rounded-[2.5rem] p-8 md:p-10 relative overflow-hidden shadow-2xl flex flex-col md:flex-row items-center justify-between gap-8 mb-8 font-black italic text-left uppercase font-black italic font-black italic leading-none ${!isPro ? 'pro-obscure' : ''}`}>
                      <div className={`flex items-center gap-5 w-full relative z-10 font-black italic leading-none font-black italic font-black italic leading-none font-black italic`}>
                         <div className="p-4 bg-white/5 rounded-2xl border border-white/10 leading-none uppercase font-black italic font-black italic leading-none font-black italic"><FileText size={32} className="text-[#25F4EE]" /></div>
                         <div><h3 className="text-2xl font-black uppercase italic leading-none text-white tracking-tight font-black italic font-black italic leading-none font-black italic">BULK ASSET INGESTION {!isPro && <Lock size={20} className="text-[#FE2C55] inline ml-2 font-black italic font-black italic leading-none font-black italic" />}</h3><p className="text-[10px] text-white/40 font-bold italic tracking-widest mt-2 leading-none uppercase font-black italic tracking-tighter uppercase font-black italic leading-none font-black italic font-black italic leading-none">Import up to 5,000 units simultaneously.</p></div>
                      </div>
                      {!isPro ? (
                        <div className="pro-lock-layer font-black italic leading-none font-black italic font-black italic leading-none font-black italic font-black italic leading-none font-black italic">
                           <p className="text-[#FE2C55] font-black uppercase tracking-widest text-[11px] mb-2 shadow-xl animate-pulse font-black italic leading-none font-black italic font-black italic leading-none font-black italic"><Lock size={12} className="inline mr-2 font-black italic font-black italic leading-none font-black italic font-black italic leading-none font-black italic"/> PRO LOCKED</p>
                           <button onClick={() => document.getElementById('marketplace-section')?.scrollIntoView({behavior: 'smooth'})} className="bg-[#25F4EE] text-black text-[9px] px-10 py-3 rounded-xl font-black uppercase italic font-black italic leading-none font-black italic font-black italic leading-none font-black italic uppercase font-black italic leading-none font-black italic font-black italic leading-none font-black italic uppercase">Unlock Ferrari Module</button>
                        </div>
                      ) : (
                   ...
                   <div id="marketplace-section" className="mb-16 mt-10 font-black italic text-left leading-none uppercase font-black italic font-black italic leading-none font-black italic">
                      <div className="flex items-center gap-3 mb-10 leading-none font-black italic uppercase font-black italic font-black italic leading-none font-black italic"><ShoppingCart size={24} className="text-[#FE2C55] leading-none font-black italic font-black italic leading-none font-black italic font-black italic leading-none font-black italic font-black italic" /><h3 className="text-xl font-black uppercase text-white text-glow-white leading-none font-black italic font-black italic leading-none font-black italic font-black italic leading-none font-black italic font-black italic">UPGRADE STATION</h3></div>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8 text-left font-black italic leading-none uppercase font-black italic font-black italic leading-none font-black italic font-black italic">
                        <div className="bg-[#111] border border-white/10 p-10 rounded-[2.5rem] group shadow-2xl hover:border-[#25F4EE] transition-colors font-black italic leading-none font-black italic">
                           ...
                           <p className="text-4xl font-black text-[#25F4EE] italic mb-8 leading-none font-black italic font-black italic">$9.00 <span className="text-xs text-white/30 italic font-black italic">/ Month</span></p>
                           <p className="text-[9px] text-white/40 mb-10 uppercase font-black italic leading-relaxed font-black italic">Unlimited Redirections & Secure Vault.</p>
                           {isMaster ? <button className="btn-strategic !bg-[#25F4EE] !text-black text-xs w-full py-4 uppercase font-black italic leading-none">ACCESS GRANTED</button> : <button className="btn-strategic !bg-white !text-black text-xs w-full py-4 uppercase font-black italic leading-none">Upgrade Now</button>}
                        </div>
                        <div className="bg-[#25F4EE]/10 border border-[#25F4EE] p-10 rounded-[2.5rem] group shadow-2xl hover:scale-[1.01] transition-transform font-black italic leading-none font-black italic">
                           <h3 className="text-3xl font-black text-white uppercase mb-4 text-[#25F4EE] leading-none font-black italic">Expert Agent</h3>
                           <p className="text-4xl font-black text-[#25F4EE] italic mb-8 leading-none font-black italic font-black italic font-black italic">$19.90 <span className="text-xs text-white/30 italic font-black italic">/ Month</span></p>
                           <p className="text-[9px] text-white/40 mb-10 uppercase font-black italic leading-relaxed font-black italic">Full IA Native Synthesis & Automated Delay.</p>
                           {isMaster ? <button className="btn-strategic !bg-[#25F4EE] !text-black text-xs w-full py-4 uppercase font-black italic leading-none">ACCESS GRANTED</button> : <button className="btn-strategic !bg-[#25F4EE] !text-black text-xs w-full py-4 uppercase font-black italic leading-none">Activate Node</button>}
                        </div>
                     </div>
                   </div>

                   {isMaster && (
                     <div className="bg-[#FE2C55]/5 border border-[#FE2C55]/20 rounded-[2.5rem] p-10 mb-16 font-black italic uppercase leading-none font-black italic font-black italic">
                        <div className="flex items-center gap-3 mb-10 font-black italic leading-none font-black italic"><Crown size={24} className="text-[#FE2C55] leading-none"/><h3 className="text-2xl uppercase tracking-tighter leading-none font-black italic font-black italic">Master Access Console</h3></div>
                        <div className="flex gap-4 mb-10 leading-none font-black italic font-black italic">
                           <input value={searchUid} onChange={e=>setSearchUid(e.target.value)} placeholder="Search UID Identity..." className="input-premium flex-1 font-black italic" />
                           <button onClick={handleAdminSearch} className="btn-strategic !bg-[#FE2C55] !text-white !w-fit px-10 font-black italic leading-none font-black italic">Scan Nodes</button>
                        </div>
                        {foundUser && (
                          <div className="bg-black/40 border border-[#FE2C55]/30 p-8 rounded-3xl animate-in zoom-in-95 flex justify-between items-center font-black italic uppercase leading-none font-black italic font-black italic">
                             <div><p className="text-xs text-white/40 mb-1 leading-none uppercase font-black italic font-black italic font-black italic">Identity: {String(foundUser.fullName)}</p><p className="text-lg text-white font-black italic leading-none mt-2 uppercase font-black italic font-black italic">Tier: {String(foundUser.tier)} | Credits: {String(foundUser.smsCredits)}</p></div>
            ...
            {view === 'auth' && (
              <div className="min-h-[80vh] flex flex-col items-center justify-center p-8 text-left font-black italic uppercase leading-none font-black italic font-black italic">
                <div className="lighthouse-neon-wrapper w-full max-w-md shadow-3xl text-left uppercase font-black italic font-black italic">
                  <div className="lighthouse-neon-content p-12 sm:p-20 relative font-black italic text-left uppercase leading-none font-black italic font-black italic">
                    <h2 className="text-3xl font-black italic mt-8 mb-12 uppercase text-white text-center font-black italic text-glow-white leading-none uppercase tracking-tighter font-black italic font-black italic">Secure Member Portal</h2>
                    <form onSubmit={handleAuthSubmit} className="space-y-6 font-black italic text-left uppercase font-black italic leading-none font-black italic">
                      ...
                      <div className="relative font-black italic leading-none uppercase font-black italic leading-none font-black italic font-black italic">
                      ...
                      <button type="submit" disabled={loading} className="btn-strategic !bg-[#25F4EE] !text-black text-[11px] mt-4 shadow-xl w-full uppercase leading-none font-black italic tracking-widest font-black italic leading-none font-black italic font-black italic">Authorize Access</button>
                      <button type="button" onClick={() => { setIsLoginMode(!isLoginMode); }} className="w-full text-[10px] text-white/20 uppercase tracking-[0.4em] mt-10 text-center hover:text-white transition-all font-black italic uppercase font-black italic leading-none font-black italic font-black italic leading-none font-black italic">{isLoginMode ? "CREATE NEW OPERATOR? REGISTER" : "ALREADY A MEMBER? LOGIN"}</button>
                    </form>
                  </div>
                </div>
              </div>
          ...
          <footer className="mt-auto pb-20 w-full space-y-16 z-10 px-10 border-t border-white/5 pt-20 text-left font-black italic uppercase leading-none font-black italic font-black italic">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-12 text-[10px] font-black uppercase italic tracking-widest text-white/30 text-left uppercase font-black italic leading-none font-black italic font-black italic font-black italic">
              <div className="flex flex-col gap-5 text-left font-black leading-none font-black italic"><span className="text-white/40 border-b border-white/5 pb-2 uppercase leading-none font-black italic">Legal</span><a href="#" className="hover:text-[#25F4EE] transition-colors leading-none font-black italic">Privacy Policy</a><a href="#" className="hover:text-[#25F4EE] transition-colors leading-none font-black italic font-black italic">Terms of Use</a></div>
              <div className="flex flex-col gap-5 text-left font-black leading-none font-black italic font-black italic font-black italic"><span className="text-white/40 border-b border-white/5 pb-2 uppercase leading-none font-black italic">Compliance</span><a href="#" className="hover:text-[#FE2C55] transition-colors leading-none font-black uppercase font-black italic">LGPD Protocol</a><a href="#" className="hover:text-[#FE2C55] transition-colors leading-none font-black uppercase font-black italic font-black italic">GDPR Node</a></div>
              <div className="flex flex-col gap-5 text-left font-black leading-none font-black italic font-black italic font-black italic"><span className="text-white/40 border-b border-white/5 pb-2 uppercase font-black italic">Network</span><a href="#" className="hover:text-[#25F4EE] transition-colors font-black italic leading-none font-black italic uppercase font-black italic">U.S. Nodes</a><a href="#" className="hover:text-[#25F4EE] transition-colors font-black italic leading-none font-black italic uppercase font-black italic">EU Nodes</a></div>
              <div className="flex flex-col gap-5 text-left font-black leading-none font-black italic font-black italic font-black italic"><span className="text-white/40 border-b border-white/5 pb-2 uppercase font-black italic">Support</span><button onClick={() => setShowSmartSupport(true)} className="hover:text-[#25F4EE] flex items-center gap-2 text-left uppercase leading-none font-black italic font-black italic font-black italic">SMART SUPPORT <Bot size={14}/></button></div>
            </div>
            <p className="text-[11px] text-white/20 font-black tracking-[8px] uppercase text-center leading-none mt-10 font-black italic uppercase font-black italic font-black italic">© 2026 ClickMoreDigital | Security Protocol</p>
          </footer>

          {showSmartSupport && (
            <div className="fixed inset-0 z-[600] flex items-center justify-center p-6 bg-black/80 backdrop-blur-md text-left font-black italic uppercase leading-none font-black italic font-black italic font-black italic">
               <div className="lighthouse-neon-wrapper w-full max-sm shadow-3xl text-left uppercase font-black italic leading-none font-black italic font-black italic">
                  <div className="lighthouse-neon-content p-10 font-black italic leading-none uppercase font-black font-black italic leading-none font-black italic font-black italic">
                     <div className="flex justify-between items-center mb-10 leading-none text-left uppercase font-black italic font-black italic leading-none font-black italic">
                        <div className="flex items-center gap-3 leading-none text-left font-black italic uppercase font-black font-black italic leading-none font-black italic"><Bot size={32} className="text-[#25F4EE] font-black leading-none font-black italic" /><span className="text-sm font-black uppercase tracking-widest text-glow-white italic uppercase font-black italic leading-none">SMART SUPPORT</span></div>
                        <button onClick={() => setShowSmartSupport(false)} className="text-white/40 hover:text-white transition-colors uppercase font-black italic leading-none font-black italic"><X size={28}/></button>
                     </div>
                     <div className="bg-black border border-white/5 p-8 rounded-3xl mb-8 min-h-[180px] flex items-center justify-center text-center font-black italic leading-relaxed uppercase font-black italic font-black italic leading-none font-black italic">
                        <p className="text-[11px] text-white/50 font-black uppercase italic tracking-widest text-center uppercase font-black italic leading-none">IA Agent online to assist your elite operation. How can I help today?</p>
                     </div>
                     <button onClick={() => {alert("Connecting node..."); setShowSmartSupport(false);}} className="btn-strategic !bg-[#25F4EE] !text-black text-xs italic uppercase font-black py-4 shadow-xl w-full uppercase shadow-2xl hover:scale-[1.02] font-black italic leading-none font-black italic font-black italic font-black italic">Connect To Node</button>
                  </div>
               </div>
            </div>
          )}
