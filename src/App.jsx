...
export default function App() {
  ...
  const handleAdminGrantTier = async (e, targetId, tierType) => {
    ...
    try {
        ...
        const updates = tierType === 'ACTIVATION_9_USD' 
          ...
          : { tier: 'PRO_SUBSCRIPTION_19_USD', smsCredits: increment(800), isSubscribed: true };
  ...
  const handleQuickSend = async (e) => {
    ...
    if (!user || !userProfile) return alert("Profile not loaded.");

    // Check credits
    const hasCredits = isMaster || isPro || (userProfile.smsCredits > 0);
    if (!hasCredits) {
      alert("Insufficient credits. Please upgrade or wait for reset.");
      return;
    }
    ...
    if (isDeviceSynced && user) {
      ...
      try {
        ...
        // Deduct credit and increment daily sent in Firestore
        if (!isMaster && !isPro) {
          ...
          // For Pro/Master, just increment dailySent if needed
          await updateDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'profile', 'data'), {
            dailySent: increment(1)
          });
        }
            ...
            if (ownerId !== ADMIN_MASTER_ID) {
              try {
                 ...
                 if (opSnap.exists()) {
                     ...
                     if (!isUnlimited) {
                         const currentCredits = data.smsCredits || 0; // treat undefined as 0
                         if (currentCredits <= 0) { 
  ...
  const handleAuthSubmit = async (e) => {
    ...
    try {
      ...
      if (isLoginMode) {
        ...
        // NATIVE MASTER ADMIN LEAD INJECTION
