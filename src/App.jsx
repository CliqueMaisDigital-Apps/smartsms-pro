...
function FAQItem({ q, a }) {
  ...
  return (
    <div className="border-b border-white/5 py-8 group cursor-pointer" onClick={() => setOpen(!open)}>
      ...
      {open && <p className="mt-6 text-sm sm:text-base text-white/60 leading-relaxed font-normal not-italic normal-case animate-in slide-in-from-top-2 text-left">{String(a)}</p>}
    </div>
  );
}
...
export default function App() {
  ...
  // Advanced Multilingual Spintax Generation based on the 6-Block / 5-Option Matrix
  ...
  const handleSendChat = async (e, directText = null) => {
    ...
    try {
        const generateHeuristicResponse = (input, historyList) => {
            ...
            // CLOSER LOGIC & UPSELL (MULTILINGUAL)
  ...
  if (view === 'bridge') {
    return (
      <div className="min-h-screen bg-[#010101] flex flex-col items-center justify-center gap-6 p-6 text-center font-black italic uppercase">
        ...
        <div className="space-y-3">
          <h2 className="text-2xl sm:text-3xl text-white tracking-tighter uppercase font-black italic">REDIRECTING TO GATEWAY...</h2>
          <p className="text-xs text-white/40 tracking-widest text-balance font-black uppercase italic">Opening your native SMS application. If it doesn't open automatically, click below.</p>
        </div>
        {captureData && (
          <a href={`sms:${captureData.to}${/iPad|iPhone|iPod/.test(navigator.userAgent) ? '&' : '?'}body=${encodeURIComponent(captureData.msg)}`} className="bg-[#25F4EE] text-black px-8 py-4 rounded-xl font-black text-sm tracking-widest shadow-[0_0_20px_#25F4EE] hover:scale-105 transition-transform uppercase italic">
            OPEN SMS APP MANUALLY
          </a>
        )}
        <p className="text-[10px] text-white/20 tracking-widest uppercase font-black italic">IDENTITY VERIFIED — ZERO-KNOWLEDGE ENCRYPTED</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#010101] text-white font-sans selection:bg-[#25F4EE] selection:text-black antialiased flex flex-col relative overflow-x-hidden">
      ...
      {globalNotifications.length > 0 && (
         <div className="bg-[#FE2C55] text-black w-full text-center py-3 px-4 flex items-center justify-center gap-3 z-[300] relative">
            ...
            <span className="text-[11px] sm:text-xs font-black uppercase italic tracking-widest">{globalNotifications[0].message}</span>
        ...
        <div className="fixed bottom-6 right-6 sm:bottom-10 sm:right-10 w-full max-w-[380px] h-[500px] bg-[#0a0a0a] border border-[#25F4EE]/30 rounded-[2rem] shadow-[0_0_40px_rgba(37,244,238,0.2)] z-[9990] flex flex-col overflow-hidden animate-in slide-in-from-bottom-4 font-sans normal-case not-italic tracking-normal text-left text-base">
          <div className="p-4 bg-[#111] border-b border-white/10 flex justify-between items-center relative overflow-hidden shrink-0">
             ...
             <div className="flex items-center gap-3 relative z-10">
                ...
                <div>
                   <h4 className="text-white text-[12px] sm:text-sm font-black italic tracking-widest leading-none uppercase">NEXUS AI SMART</h4>
                   <p className="text-[9px] sm:text-[10px] text-[#25F4EE] tracking-[0.2em] font-black italic mt-1 uppercase">SYSTEM ONLINE</p>
                </div>
             </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 custom-scrollbar bg-black/50 space-y-4">
            {chatMessages.length === 0 && (
               <div className="text-center py-6">
                 ...
                 <p className="text-[10px] sm:text-[11px] text-white/30 tracking-widest font-black uppercase italic">NEXUS AGENT IS READY TO ASSIST</p>
               </div>
            )}
                  ...
                  {msg.buttons && (
                     <div className="mt-4 flex flex-col gap-2">
                        {msg.buttons.map((btn, bIdx) => (
                           <button key={bIdx} onClick={() => handleChatButtonAction(btn.action)} className="w-full text-center py-2 px-3 bg-white/5 hover:bg-[#25F4EE]/20 border border-white/10 hover:border-[#25F4EE]/50 rounded-lg text-[10px] sm:text-[11px] font-black italic tracking-widest uppercase transition-all text-[#25F4EE]">
                              {btn.label}
                           </button>
                        ))}
                     </div>
      ...
      <nav className="fixed top-0 left-0 right-0 h-16 bg-black/80 backdrop-blur-xl border-b border-white/5 z-[200] px-6 flex justify-between items-center transition-all">
        <div className="flex items-center gap-3 cursor-pointer relative z-[210]" onClick={() => { setView('home'); setIsMenuOpen(false); }}>
          ...
          <span className="text-xl font-black italic uppercase tracking-tighter text-white mt-1">SMART SMS PRO</span>
        </div>

        <div className="hidden md:flex items-center gap-10 text-[11px] sm:text-xs tracking-widest uppercase font-black italic relative z-[210]">
           {!user ? (
             <>
      ...
      <div className={`md:hidden fixed inset-0 z-[150] bg-[#010101]/95 backdrop-blur-3xl transition-all duration-400 ease-[cubic-bezier(0.23,1,0.32,1)] flex flex-col pt-24 px-6 pb-12 overflow-y-auto ${isMenuOpen ? 'opacity-100 pointer-events-auto translate-y-0' : 'opacity-0 pointer-events-none -translate-y-8'}`}>
        <div className="flex flex-col gap-5 flex-1 mt-4">
          ...
          {!user ? (
            <>
              <button onClick={() => { setIsWelcomeTrial(false); setIsLoginMode(true); setView('auth'); setIsMenuOpen(false); }} className="bg-transparent border-2 border-[#25F4EE] text-[#25F4EE] hover:bg-[#25F4EE]/10 p-6 rounded-2xl text-[11px] sm:text-xs tracking-[0.15em] font-black uppercase italic transition-all flex items-center justify-center gap-3 w-full shadow-[0_0_15px_rgba(37,244,238,0.2)]"><Lock size={20}/> SECURE MEMBER PORTAL</button>
              <button onClick={() => { setIsWelcomeTrial(false); setIsLoginMode(false); setView('auth'); setIsMenuOpen(false); }} className="bg-gradient-to-r from-[#25F4EE] to-[#1AB5B0] text-black p-6 rounded-2xl text-[11px] sm:text-xs tracking-[0.15em] font-black uppercase italic shadow-[0_0_30px_rgba(37,244,238,0.4)] flex items-center justify-center gap-3 w-full"><Rocket size={20}/> JOIN NETWORK & START</button>
            </>
          ) : (
            <>
              <button onClick={() => { setView('dashboard'); setIsMenuOpen(false); }} className={`p-6 rounded-2xl border ${view === 'dashboard' ? 'bg-[#25F4EE]/10 border-[#25F4EE] text-[#25F4EE]' : 'bg-white/5 border-white/10 text-white hover:border-[#25F4EE]/50'} text-[11px] sm:text-xs tracking-[0.15em] font-black uppercase italic transition-all flex items-center justify-center gap-3 w-full`}><LayoutDashboard size={20}/> ACCESS OPERATOR HUB</button>
              <button onClick={() => { setShowSmartSupport(true); setIsMenuOpen(false); }} className="p-6 rounded-2xl border bg-white/5 border-white/10 text-white hover:border-[#25F4EE]/50 text-[11px] sm:text-xs tracking-[0.15em] font-black uppercase italic transition-all flex items-center justify-center gap-3 w-full"><Bot size={20}/> NEXUS AI SMART SUPPORT</button>
            </>
          )}
        </div>
        ...
        {user && (
           <div className="mt-auto pt-8 border-t border-white/5">
              <button onClick={() => { signOut(auth).then(()=>{setView('home'); setIsMenuOpen(false);}) }} className="w-full p-6 rounded-2xl border bg-[#FE2C55]/10 border-[#FE2C55]/30 text-[#FE2C55] hover:bg-[#FE2C55]/20 text-[11px] sm:text-xs tracking-[0.15em] font-black uppercase italic transition-all flex items-center justify-center gap-3 shadow-[0_0_20px_rgba(254,44,85,0.1)]"><LogOut size={20}/> DISCONNECT SECURITY GATEWAY</button>
           </div>
        )}
      </div>
        ...
        {view === 'home' && (
          <div className="w-full max-w-[580px] mx-auto px-4 z-10 relative text-center pt-12 animate-in fade-in duration-300">
            <header className="mb-16 text-center flex flex-col items-center">
              <div className="lighthouse-neon-wrapper mb-5"><div className="lighthouse-neon-content px-12 py-5"><h1 className="text-4xl sm:text-5xl text-white text-glow-white tracking-tight uppercase font-black italic">SMART SMS PRO</h1></div></div>
              <p className="text-[10px] sm:text-xs text-white/40 font-black italic uppercase tracking-[0.3em] sm:tracking-[0.4em] text-center px-4 leading-loose">HIGH-END REDIRECTION PROTOCOL<br/>60 FREE SECURE CONNECTIONS</p>
            </header>
            ...
            <main className="space-y-10 pb-20 text-left">
              ...
              <div className="lighthouse-neon-wrapper shadow-3xl mx-2 sm:mx-0">
                <div className="lighthouse-neon-content p-8 sm:p-14 text-left space-y-10">
                  <div className="flex items-center gap-3 mb-2"><div className="w-2.5 h-2.5 rounded-full bg-amber-500 animate-pulse shadow-[0_0_10px_#f59e0b]"></div><h3 className="text-xs sm:text-sm tracking-widest text-white/60 font-black italic uppercase">SMART CONNECTION PROTOCOL</h3></div>
                  <div className="space-y-4">
                     <label className="text-[10px] sm:text-xs text-white/40 ml-1 tracking-widest block font-black italic uppercase">RECIPIENT (TARGET NUMBER) <span className="text-[#25F4EE] ml-2 opacity-50 text-[9px] sm:text-[10px]">EX: +1 999 999 9999</span></label>
                  ...
                  <div className="space-y-4">
                     <label className="text-[10px] sm:text-xs text-white/40 ml-1 tracking-widest block font-black italic uppercase">HOST IDENTITY (NAME OR COMPANY)</label>
                  ...
                  <div className="space-y-4">
                     <div className="flex justify-between items-center"><label className="text-[10px] sm:text-xs text-white/40 ml-1 tracking-widest block font-black italic uppercase">SMS MESSAGE PAYLOAD</label><span className="text-[9px] sm:text-[10px] text-white/20 font-black italic">{genMsg.length}/{MSG_LIMIT}</span></div>
                     <div className="flex items-center gap-2 font-black italic mb-3 mt-1">
                     ...
                     {isGenMsgForbidden && <p className="text-[11px] sm:text-xs text-[#FE2C55] mt-3 font-black italic tracking-widest animate-pulse text-center leading-relaxed">⚠️ ZERO TOLERANCE POLICY: PROHIBITED WORDS DETECTED. PLEASE CORRECT.</p>}
                  </div>
                  
                  {showInstructions && (
                    <div className="p-6 sm:p-8 bg-white/[0.03] border border-[#25F4EE]/20 rounded-2xl animate-in slide-in-from-top-2">
                       <h5 className="text-[11px] sm:text-[12px] text-[#25F4EE] mb-4 text-center font-black italic uppercase">STRATEGIC DEPLOYMENT GUIDE:</h5>
        ...
        {view === 'dashboard' && (
          <div className="w-full max-w-7xl mx-auto py-8 sm:py-12 px-4 sm:px-8 animate-in fade-in duration-300">
            ...
            <div className="flex flex-col lg:flex-row justify-between lg:items-end gap-8 mb-12 sm:mb-16 text-left">
              <div>
                <h2 className="text-5xl sm:text-6xl md:text-7xl tracking-tighter drop-shadow-[0_0_15px_rgba(255,255,255,0.8)] text-white font-black italic uppercase">OPERATOR HUB</h2>
                <div className="flex flex-wrap items-center gap-4 mt-5">
                   <span className={`text-[10px] sm:text-xs px-5 py-2 rounded-full border tracking-widest uppercase italic ${isMaster ? 'bg-[#25F4EE]/10 border-[#25F4EE] text-[#25F4EE] shadow-[0_0_15px_rgba(37,244,238,0.3)] animate-pulse font-black' : 'bg-white/5 border-white/10 text-white/40 font-black'}`}>
                   ...
                   {isPro && <span className="text-[9px] sm:text-[10px] text-amber-500 tracking-widest animate-pulse font-black uppercase italic">● LIVE PROTOCOL ACTIVE</span>}
                </div>
              </div>
              <div className="flex items-stretch gap-4 sm:gap-5 flex-wrap text-center uppercase italic">
            ...
            {isMaster && (
                <div className="bg-[#0a0a0a] border border-amber-500/30 p-8 sm:p-10 rounded-3xl sm:rounded-[2.5rem] shadow-[0_0_30px_rgba(245,158,11,0.1)] flex flex-col relative overflow-hidden mb-10">
                  <h3 className="text-xl sm:text-2xl text-white mb-6 flex items-center gap-4 font-black uppercase italic"><BellRing className="text-amber-500 animate-pulse" size={26} /> GLOBAL PLATFORM BROADCAST</h3>
                  <form onSubmit={handleBroadcastPush} className="flex gap-4 flex-col sm:flex-row items-stretch sm:items-center">
                    ...
                    <button type="submit" disabled={loading} className="shrink-0 h-fit bg-amber-500 text-black font-black text-xs sm:text-sm tracking-widest px-10 py-[1.2rem] rounded-xl hover:bg-amber-400 transition-all shadow-[0_0_15px_rgba(245,158,11,0.3)] disabled:opacity-50 flex items-center justify-center gap-3 uppercase italic">
                      <Send size={18}/> DEPLOY
                    </button>
                  </form>
                </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-5 sm:gap-8 mb-10 uppercase italic">
              {[
            ...
            <div className="bg-[#0a0a0a] border border-[#25F4EE]/20 rounded-3xl sm:rounded-[2.5rem] shadow-[0_0_30px_rgba(37,244,238,0.1)] p-8 sm:p-10 mb-12 flex flex-col relative overflow-hidden">
               ...
               <div className="flex items-center gap-4 mb-8">
                  ...
                  <h3 className="text-2xl sm:text-3xl text-white tracking-tight font-black uppercase italic">COGNITIVE ANALYTICS & TELEMETRY</h3>
               </div>
               <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
                  ...
                  <div className="bg-[#111] border border-white/5 p-6 rounded-2xl flex flex-col justify-between hover:border-[#25F4EE]/30 transition-all">
                     <div className="flex items-center justify-between mb-4">
                        <span className="text-[10px] sm:text-[11px] text-white/50 tracking-widest font-black uppercase italic">GLOBAL CLICK-THROUGH</span>
                  ...
                  <div className="bg-[#111] border border-white/5 p-6 rounded-2xl flex flex-col justify-between hover:border-[#25F4EE]/30 transition-all">
                     <div className="flex items-center justify-between mb-4">
                        <span className="text-[10px] sm:text-[11px] text-white/50 tracking-widest font-black uppercase italic">BEHAVIORAL PROJECTION</span>
                     ...
                     <p className="text-[9px] sm:text-[10px] text-amber-500 tracking-widest font-black uppercase mt-4 italic">ESTIMATED CONVERSIONS (24H)</p>
                  </div>
                  {/* Metric 3 */}
                  <div className="bg-[#111] border border-white/5 p-6 rounded-2xl flex flex-col justify-between hover:border-[#25F4EE]/30 transition-all">
                     <div className="flex items-center justify-between mb-4">
                        <span className="text-[10px] sm:text-[11px] text-white/50 tracking-widest font-black uppercase italic">DEVICE HEURISTICS</span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 sm:gap-10 mb-20">
              ...
              <div className="lg:col-span-1 space-y-8 sm:space-y-10 flex flex-col">
                <div className="bg-[#0a0a0a] border border-white/10 p-8 sm:p-10 rounded-3xl sm:rounded-[2.5rem] shadow-2xl flex flex-col relative overflow-hidden flex-1">
                  <h3 className="text-xl sm:text-2xl text-white mb-8 flex items-center gap-3 font-black uppercase italic"><Zap className="text-[#25F4EE]" size={24} /> INSTANT PAYLOAD DISPATCH</h3>
                  <form onSubmit={handleQuickSend} className="space-y-5 sm:space-y-6 flex flex-col flex-1">
                    <div>
                      <label className="block text-[10px] sm:text-[11px] text-white/40 tracking-widest mb-3 font-black uppercase italic">RECIPIENT (TARGET NUMBER)</label>
                    ...
                    <div className="flex-1 flex flex-col">
                      <label className="block text-[10px] sm:text-[11px] text-white/40 tracking-widest mb-3 font-black uppercase italic">SMS MESSAGE PAYLOAD</label>
                    ...
                    <button type="submit" disabled={loading} className="btn-strategic !bg-[#25F4EE] !text-black text-[11px] sm:text-xs w-full mt-6 py-5 shadow-[0_0_15px_rgba(37,244,238,0.2)] font-black uppercase italic">
                ...
                <div className={`bg-[#0a0a0a] border border-white/10 rounded-3xl sm:rounded-[2.5rem] p-8 sm:p-10 shadow-2xl relative overflow-hidden ${!isPro ? 'pro-obscure' : ''}`}>
                   <div className={`flex items-center justify-between w-full relative z-10`}>
                      <div><h3 className="text-xl sm:text-2xl text-white mb-2 flex items-center gap-3 font-black uppercase italic"><UploadCloud size={24} className="text-[#25F4EE]"/> MASS INGESTION HUB {!isPro && <Lock size={18} className="text-[#FE2C55]" />}</h3><p className="text-[9px] sm:text-[10px] text-white/40 tracking-widest font-black uppercase italic">IMPORT 5K UNITS.</p></div>
                   ...
                   {!isPro && (
                     <div className="pro-lock-layer p-6">
                        <p className="text-[#FE2C55] tracking-widest text-[10px] sm:text-[11px] mb-3 animate-pulse font-black uppercase italic"><Lock size={14} className="inline mr-2"/> PRO LOCKED</p>
                        <button onClick={() => document.getElementById('marketplace-section')?.scrollIntoView({behavior: 'smooth'})} className="bg-white/10 text-white border border-white/20 text-[9px] sm:text-[10px] px-8 py-3.5 rounded-xl whitespace-nowrap font-black uppercase italic">UNLOCK</button>
                     </div>
                   )}
              ...
              <div className="lg:col-span-2 bg-[#0a0a0a] rounded-3xl sm:rounded-[2.5rem] border border-white/10 shadow-3xl overflow-hidden flex flex-col h-full min-h-[500px] sm:min-h-[600px]">
                 <div className="p-8 sm:p-10 border-b border-white/10 flex flex-col sm:flex-row justify-between sm:items-center gap-6 bg-[#111]">
                    <div className="flex items-center gap-4">
                       ...
                       <h3 className="text-2xl sm:text-3xl text-white tracking-tight leading-tight font-black uppercase italic">{isMaster ? 'PREMIUM CRM & NETWORK MAP' : 'RECENT ACTIVITY LOGS'}</h3>
                    </div>
                 </div>
                   ...
                   {isMaster ? (
                     <table className="w-full text-left font-sans font-medium not-italic normal-case min-w-[700px]">
                       <thead className="bg-[#111] sticky top-0 z-10 uppercase border-b border-white/5">
                         <tr>
                           <th className="px-8 sm:px-10 py-5 sm:py-6 text-[10px] sm:text-[11px] text-white/50 tracking-widest font-black italic">SUBSCRIBER ALIAS (NICKNAME)</th>
                           <th className="px-8 sm:px-10 py-5 sm:py-6 text-[10px] sm:text-[11px] text-white/50 tracking-widest text-center font-black italic">CAPTURED LEADS</th>
                           <th className="px-8 sm:px-10 py-5 sm:py-6 text-[10px] sm:text-[11px] text-white/50 tracking-widest text-right font-black italic">MASTER FOLDER STATUS</th>
                         </tr>
                       </thead>
                                ...
                                <tr onClick={() => setExpandedAdminRow(expandedAdminRow === sub.id ? null : sub.id)} className={`hover:bg-white/[0.02] transition-colors cursor-pointer group ${expandedAdminRow === sub.id ? 'bg-white/[0.03]' : ''}`}>
                                   <td className="px-8 sm:px-10 py-6 sm:py-8">
                                      <div className="flex items-center gap-4">
                                          ...
                                          <div>
                                              <p className="text-sm sm:text-base text-[#25F4EE] tracking-wider font-black uppercase italic">{String(sub.nickname).toUpperCase()}</p>
                                              <p className="text-[10px] sm:text-[11px] text-white/40 tracking-widest font-mono mt-1.5 uppercase">{sub.email} | {sub.tier}</p>
                                          </div>
                                      </div>
                                   </td>
                                   <td className="px-8 sm:px-10 py-6 sm:py-8 text-center text-sm sm:text-base text-white font-black uppercase italic">
                                   ...
                                   <td className="px-8 sm:px-10 py-6 sm:py-8 flex justify-end gap-2 sm:gap-3 mt-2 sm:mt-3 uppercase italic">
                                                ...
                                                {sub.leads.length > 0 ? (
                                                    <table className="w-full text-left font-sans font-medium not-italic normal-case">
                                                       <thead className="text-[9px] sm:text-[10px] text-white/30 uppercase tracking-widest border-b border-white/5">
                                                           <tr>
                                                               <th className="pb-4 px-5 font-black italic">TARGET NUMBER</th>
                                                               <th className="pb-4 px-5 font-black italic">IDENTITY</th>
                                                               <th className="pb-4 px-5 font-black italic">FOLDER</th>
                                                               <th className="pb-4 px-5 text-right font-black italic">ACTIONS (CRM)</th>
                                                           </tr>
                                                       </thead>
                                                       <tbody className="divide-y divide-white/5">
                                                           {sub.leads.map(l => (
                                                              <tr key={l.id} className="hover:bg-white/5 group">
                                                                 ...
                                                                 <td className="py-4 px-5 text-xs sm:text-sm text-white">{l.nome_cliente} {sub.id === 'AI_SMART_CHAT' && <span className="ml-3 text-[9px] tracking-widest text-black bg-amber-500 px-2 py-0.5 rounded-sm uppercase italic">NEXUS AGENT</span>}</td>
                                                                 <td className="py-4 px-5 text-xs sm:text-sm">
                                                                    <select
                                                                      ...
                                                                      className="bg-[#111] border border-white/10 text-white/50 text-[10px] sm:text-[11px] px-3 py-1.5 rounded-lg outline-none font-black italic tracking-widest appearance-none cursor-pointer hover:border-[#25F4EE]/30 transition-colors uppercase"
                                                                    >
                                                                      {folders.map(f => (
                                                    ...
                                                    <p className="text-xs sm:text-sm text-white/30 tracking-widest text-center py-8 font-black uppercase italic">NO LEADS CAPTURED BY THIS GATEWAY YET.</p>
                                                )}
                                            </div>
                                        </td>
                                    </tr>
                     ...
                     <>
                       {logs.length > 0 ? (
                         <table className="w-full text-left font-sans font-medium not-italic normal-case min-w-[500px]">
                           <thead className="bg-[#111] sticky top-0 z-10 uppercase border-b border-white/5">
                             <tr>
                               <th className="px-6 sm:px-8 py-5 sm:py-6 text-[10px] sm:text-[11px] text-white/50 tracking-widest font-black italic">RECIPIENT (TARGET NUMBER)</th>
                               <th className="px-6 sm:px-8 py-5 sm:py-6 text-[10px] sm:text-[11px] text-white/50 tracking-widest font-black italic">HOST IDENTITY</th>
                               <th className="px-6 sm:px-8 py-5 sm:py-6 text-[10px] sm:text-[11px] text-white/50 tracking-widest font-black italic">STATUS</th>
                               <th className="px-6 sm:px-8 py-5 sm:py-6 text-[10px] sm:text-[11px] text-white/50 tracking-widest text-right font-black italic">TIMESTAMP</th>
                             </tr>
                           </thead>
                           <tbody className="divide-y divide-white/5">
                             {logs.map(l => (
                         ...
                         <div className="flex flex-col items-center justify-center h-full p-12 sm:p-24 opacity-20 text-center"><Lock size={56} className="mb-5 text-white" /><p className="text-[11px] sm:text-xs tracking-widest font-black uppercase italic">VAULT STANDBY</p><p className="text-[9px] sm:text-[10px] mt-3 font-sans font-medium not-italic normal-case text-balance">NO ACTIVE INTERCEPTIONS.</p></div>
                       )}
                     </>
                   )}
                 </div>
                 {!isPro && !isMaster && logs.length > 0 && (
                   <div className="p-8 sm:p-10 bg-gradient-to-t from-[#FE2C55]/10 to-transparent border-t border-[#FE2C55]/20 flex flex-col sm:flex-row items-center justify-between gap-5 uppercase italic">
            ...
            <div className={`bg-[#0a0a0a] border ${isAiObjectiveForbidden ? 'border-[#FE2C55] shadow-[0_0_30px_rgba(254,44,85,0.2)]' : 'border-white/10'} rounded-3xl sm:rounded-[2.5rem] p-8 sm:p-10 md:p-12 shadow-2xl mb-10 relative overflow-hidden transition-all ${!isPro ? 'pro-obscure' : ''}`}>
              <div className={`flex flex-col text-left`}>
                 <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-5 md:gap-8 mb-8 sm:mb-10">
                    <div className="flex items-center gap-4 sm:gap-5 text-left">
                      ...
                      <div>
                        <h3 className="text-2xl sm:text-3xl text-white tracking-tight leading-tight font-black uppercase italic">NEXUS SMART SHUFFLE ENGINE {!isPro && <Lock size={20} className="text-[#FE2C55] inline ml-2" />}</h3>
                        <p className="text-[10px] sm:text-[11px] text-white/40 tracking-widest mt-2 sm:mt-3 font-black uppercase italic">AUTOMATED LINGUISTIC SCRAMBLING TO OBLITERATE CARRIER FILTER BLOCKS.</p>
                      </div>
                    </div>
                    <button onClick={() => setShowHelpModal(true)} className="flex items-center justify-center gap-3 bg-[#25F4EE]/10 text-[#25F4EE] px-6 py-4 rounded-xl text-[10px] sm:text-[11px] font-black uppercase italic hover:bg-[#25F4EE]/20 transition-all border border-[#25F4EE]/30 w-full md:w-auto shrink-0 mt-3 md:mt-0">
                 ...
                 {isReviewMode ? (
                   <div className="animate-in slide-in-from-bottom-4 fade-in duration-300">
                     <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 sm:gap-0 mb-6 border-b border-white/10 pb-5">
                        <div className="flex items-center gap-3">
                           ...
                           <h4 className="text-xl sm:text-2xl text-white font-black uppercase italic">PAYLOAD REVIEW ENGINE</h4>
                        </div>
                        <p className="text-[10px] sm:text-[11px] text-white/50 tracking-widest font-black uppercase italic">{stagedQueue.length} VARIATIONS PENDING</p>
                     </div>
                        ...
                        {stagedQueue.map((task, idx) => (
                           <div key={task.id || idx} className={`bg-[#111] border border-white/5 rounded-2xl p-5 sm:p-6 transition-colors flex flex-col h-[160px] sm:h-[180px] ${isDispatching && idx === 0 ? 'border-[#25F4EE] shadow-[0_0_15px_rgba(37,244,238,0.3)] animate-pulse' : 'hover:border-[#25F4EE]/30 group'}`}>
                              <div className="flex justify-between items-center mb-3 sm:mb-4">
                                <span className="text-[#25F4EE] text-[9px] sm:text-[10px] font-black tracking-widest uppercase italic">VARIATION {sessionSentCount + idx + 1}</span>
                     ...
                     <div className="flex flex-col lg:flex-row justify-between lg:items-center gap-5 mt-3 bg-[#111] p-6 sm:p-8 rounded-2xl sm:rounded-[2rem] border border-white/5 shadow-inner">
                        <div className="flex items-center justify-between sm:justify-start gap-4 px-2 sm:px-3 w-full lg:w-auto border-b sm:border-b-0 border-white/5 pb-4 sm:pb-0">
                           <div className="flex items-center gap-3 sm:gap-4">
                              ...
                              <span className="text-[10px] sm:text-[11px] text-white/50 tracking-widest font-black uppercase italic mt-0.5 whitespace-nowrap">TRANSMISSION PACING:</span>
                           </div>
                           <select disabled={isDispatching} value={sendDelay} onChange={e => setSendDelay(Number(e.target.value))} className="bg-transparent text-[#10B981] text-[11px] sm:text-sm font-black outline-none cursor-pointer border-b border-[#10B981]/30 pb-1 appearance-none text-right sm:text-left uppercase italic">
                        ...
                        <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-5 w-full lg:w-auto uppercase italic">
                   ...
                   <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-10 text-left">
                      <div className="space-y-5 flex flex-col">
                         <div className="flex items-center justify-between gap-4 bg-black/40 border border-white/5 p-5 rounded-2xl mb-3 flex-wrap">
                           <div className="flex items-center gap-4">
                             ...
                             <div>
                               <p className="text-[9px] sm:text-[10px] text-white/50 tracking-widest font-black uppercase italic mb-1.5">DISPATCH DELAY</p>
                               <select disabled={!isPro} value={sendDelay} onChange={e => setSendDelay(Number(e.target.value))} className="bg-transparent text-[#10B981] text-[11px] sm:text-xs font-black outline-none cursor-pointer w-full appearance-none uppercase italic">
                           ...
                           <div className="border-l border-white/10 pl-5">
                             <p className="text-[9px] sm:text-[10px] text-white/50 tracking-widest font-black uppercase italic mb-1.5">TARGET BATCH FOLDER</p>
                             <div className="flex items-center gap-3">
                               <select disabled={!isPro} value={selectedFolder} onChange={e => setSelectedFolder(e.target.value)} className="bg-transparent text-amber-500 text-[11px] sm:text-xs font-black outline-none cursor-pointer w-full appearance-none uppercase italic">
                                  {folders.map(f => (
                         ...
                         <div className="flex items-center gap-3 font-black italic pt-2">
                      ...
                      <div className="bg-[#111] border border-white/5 rounded-2xl sm:rounded-[2rem] flex flex-col items-center justify-center p-8 sm:p-10 min-h-[220px] sm:min-h-[250px] text-center shadow-inner relative overflow-hidden mt-5 lg:mt-0 uppercase italic">
                        {smsQueueCount > 0 ? (
                          <div className="flex flex-col items-center justify-center w-full animate-in fade-in zoom-in-95">
                             ...
                             {nodeWarningActive && (
                               <div className="mt-8 p-5 w-full bg-[#FE2C55]/10 border border-[#FE2C55]/30 rounded-2xl text-left animate-in slide-in-from-bottom-2">
                                 <p className="text-[11px] sm:text-xs text-[#FE2C55] font-black tracking-widest flex items-center gap-2 mb-2 uppercase italic"><WifiOff size={14}/> DEVICE DISCONNECTED</p>
                        ...
                        {!smsQueueCount && (
                          <div className="absolute bottom-5 left-5 right-5 p-4 sm:p-5 bg-white/[0.02] border border-white/5 rounded-2xl text-left hidden sm:block">
                             <p className="text-[10px] sm:text-[11px] text-[#10B981] font-black tracking-widest flex items-center gap-2.5 mb-2 uppercase italic"><Wifi size={14} className="animate-pulse"/> GHOST PROTOCOL ACTIVE</p>
              ...
              {!isPro && <div className="pro-lock-layer p-5"><p className="text-[#FE2C55] tracking-widest text-[12px] sm:text-sm mb-4 shadow-xl animate-pulse font-black uppercase italic"><Lock size={14} className="inline mr-2.5"/> PRO LOCKED</p><button onClick={() => document.getElementById('marketplace-section')?.scrollIntoView({behavior: 'smooth'})} className="bg-[#25F4EE] text-black text-[10px] sm:text-[11px] px-10 py-4 rounded-xl whitespace-nowrap font-black uppercase italic">UNLOCK NEXUS AGENT</button></div>}
            </div>
            ...
            <div className="bg-[#0a0a0a] border border-white/10 rounded-3xl sm:rounded-[2.5rem] overflow-hidden shadow-3xl mb-16 flex flex-col text-left">
              <div className="p-8 sm:p-10 border-b border-white/10 flex justify-between items-center bg-[#111]"><div className="flex items-center gap-3"><Radio size={24} className="text-[#25F4EE]" /><h3 className="text-xl sm:text-2xl tracking-tight font-black uppercase italic">ACTIVE PROTOCOLS INVENTORY</h3></div></div>
              <div className="min-h-[150px] sm:min-h-[250px] max-h-[40vh] overflow-y-auto bg-black custom-scrollbar">
                {linksHistory.length > 0 ? linksHistory.map(l => (
                  <div key={l.id} className="p-6 sm:p-10 border-b border-white/5 flex flex-col md:flex-row justify-between md:items-center gap-5 sm:gap-8 hover:bg-white/[0.02] transition-colors">
                    <div className="flex-1 overflow-hidden">
                       <p className="text-[11px] sm:text-xs text-white/30 mb-2 flex items-center gap-2.5 tracking-widest uppercase italic"><Calendar size={14}/> {l.created_at && typeof l.created_at.toDate === 'function' ? l.created_at.toDate().toLocaleString('en-US') : 'Syncing Gateway...'}</p>
                ...
                )) : <div className="p-20 sm:p-24 text-center opacity-20 uppercase italic"><Lock size={48} className="mx-auto mb-5" /><p className="text-[11px] sm:text-[12px] tracking-widest font-black">NO PROTOCOLS ESTABLISHED</p></div>}
              </div>
            </div>

            {/* UPGRADE STATION */}
            <div id="marketplace-section" className="mb-14 sm:mb-20 mt-10 sm:mt-12 text-left">
               <div className="flex items-center gap-3 mb-10 sm:mb-12"><ShoppingCart size={28} className="text-[#FE2C55]"/><h3 className="text-2xl sm:text-3xl text-white text-glow-white font-black uppercase italic">NEXUS UPGRADE HUB</h3></div>
               <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-10 mb-8 sm:mb-10 text-left">
                 <div className="bg-[#111] border border-white/10 p-10 sm:p-12 rounded-3xl sm:rounded-[3rem] group shadow-2xl hover:border-[#25F4EE]/50 transition-colors">
                    <h3 className="text-3xl sm:text-4xl text-white mb-4 sm:mb-5 font-black tracking-tight uppercase italic">NEXUS ROUTING PRO</h3>
                    ...
                    {isMaster ? <button className="btn-strategic !bg-[#25F4EE] !text-black text-[11px] sm:text-xs w-full py-5 sm:py-6 font-black uppercase italic">UNLIMITED ACCESS</button> : <button onClick={() => window.location.href = STRIPE_LINKS.NEXUS_ROUTING_PRO} className="btn-strategic !bg-white !text-black text-[11px] sm:text-xs w-full py-5 sm:py-6 hover:shadow-[0_0_20px_rgba(255,255,255,0.3)] font-black uppercase italic">UPGRADE NOW</button>}
                 </div>
                 <div className="bg-[#25F4EE]/10 border border-[#25F4EE] p-10 sm:p-12 rounded-3xl sm:rounded-[3rem] group shadow-[0_0_30px_rgba(37,244,238,0.15)] hover:scale-[1.01] transition-transform">
                    <h3 className="text-3xl sm:text-4xl text-white mb-4 sm:mb-5 text-[#25F4EE] font-black tracking-tight uppercase italic">NEXUS AUTOMATION ENGINE</h3>
                    ...
                    {isMaster ? <button className="btn-strategic !bg-[#25F4EE] !text-black text-[11px] sm:text-xs w-full py-5 sm:py-6 font-black uppercase italic">UNLIMITED ACCESS</button> : <button onClick={() => window.location.href = STRIPE_LINKS.NEXUS_AUTOMATION_ENGINE} className="btn-strategic !bg-[#25F4EE] !text-black text-[11px] sm:text-xs w-full py-5 sm:py-6 shadow-[0_0_20px_rgba(37,244,238,0.3)] font-black uppercase italic">ACTIVATE GATEWAY</button>}
                 </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5 sm:gap-8 mb-16 sm:mb-20 uppercase italic">
                 {[
           ...
           <div className="max-w-7xl mx-auto w-full flex flex-col gap-8">
              <div className="flex flex-col xl:flex-row items-start xl:items-center justify-between gap-8">
                 <div className="flex-1 text-left">
                   <h4 className="text-[#25F4EE] font-black text-sm sm:text-base mb-4 tracking-widest uppercase italic flex items-center gap-3"><ShieldCheck size={24}/> LEGAL & PRIVACY CONSENT</h4>
                 ...
                 <div className="flex flex-col sm:flex-row gap-4 w-full xl:w-auto shrink-0 uppercase italic">
              ...
              {showCookieDetails && (
                 <div className="p-8 sm:p-10 bg-[#111] border border-white/5 rounded-2xl animate-in fade-in zoom-in-95 text-left mt-3">
                    <p className="text-sm sm:text-base text-white/60 font-sans not-italic normal-case leading-loose">
                      <span className="text-amber-500 font-black block mb-3 uppercase italic tracking-widest text-[11px] sm:text-xs">ZERO TOLERANCE & COMPLIANCE PROTOCOL</span>
                      ...
                      <br/><br/>
                      <span className="text-[#25F4EE] font-black block mb-3 uppercase italic tracking-widest text-[11px] sm:text-xs">ESSENTIAL COOKIES (MANDATORY)</span>
      ...
      <footer className="mt-auto pb-16 sm:pb-24 w-full z-[100] px-8 sm:px-12 border-t border-white/5 pt-16 sm:pt-24 text-left bg-[#010101] relative">
        <div className="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-10 sm:gap-14 text-[11px] sm:text-[12px] tracking-[0.15em] sm:tracking-widest text-white/30 font-black uppercase italic">
        ...
        <div className="max-w-7xl mx-auto mt-16 sm:mt-20 pt-10 border-t border-white/5 flex justify-center">
           <p className="text-[10px] sm:text-[12px] text-white/20 tracking-[0.3em] sm:tracking-[0.5em] text-center w-full px-5 text-glow-white font-black uppercase italic">© 2026 CLICKMORE DIGITAL | EXCLUSIVE SECURITY PROTOCOL</p>
        </div>
      </footer>
          ...
          <div className="bg-[#0a0a0a] border border-[#25F4EE]/30 w-full max-w-2xl rounded-[2rem] sm:rounded-[2.5rem] shadow-[0_0_50px_rgba(37,244,238,0.15)] flex flex-col max-h-[85vh] overflow-hidden relative">
             ...
             <div className="p-8 sm:p-10 border-b border-white/10 flex justify-between items-center bg-[#111] shrink-0">
                <div className="flex items-center gap-4 sm:gap-5">
                   ...
                   <h3 className="text-2xl sm:text-3xl text-white tracking-tight font-black uppercase italic">{legalContent === 'ZERO' ? 'ZERO TOLERANCE POLICY' : 'LEGAL PROTOCOL'}</h3>
                </div>
             ...
             <div className="p-8 sm:p-12 overflow-y-auto custom-scrollbar flex-1 bg-gradient-to-b from-[#111] to-black">
                {legalContent === 'ZERO' ? (
                  <p className="text-base sm:text-lg text-white/70 font-sans not-italic normal-case leading-loose sm:leading-loose">
                    <span className="text-amber-500 font-bold block mb-5 uppercase italic tracking-widest text-[11px] sm:text-xs">STRICT COMPLIANCE ENFORCEMENT</span>
                ...
                <div className="mt-10 sm:mt-14 p-6 sm:p-8 bg-[#25F4EE]/5 border border-[#25F4EE]/20 rounded-2xl sm:rounded-3xl">
                   <p className="text-[11px] sm:text-[12px] text-[#25F4EE] font-black tracking-widest uppercase italic mb-3">ENCRYPTED AT REST</p>
             ...
             <div className="p-8 sm:p-10 border-t border-white/10 bg-black shrink-0 flex justify-end">
                <button onClick={() => setLegalContent(null)} className="w-full sm:w-auto px-10 sm:px-12 py-4 sm:py-5 bg-[#25F4EE] text-black text-[12px] sm:text-sm font-black tracking-widest uppercase italic rounded-xl hover:scale-[1.02] transition-transform shadow-[0_0_20px_rgba(37,244,238,0.3)]">ACKNOWLEDGE & CLOSE</button>
             </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[600] bg-[#010101]/95 backdrop-blur-xl flex flex-col items-center justify-center p-6 text-center animate-in fade-in zoom-in-95">
          <div className="lighthouse-neon-wrapper w-full max-w-sm shadow-[0_0_50px_rgba(37,244,238,0.3)]">
            <div className="lighthouse-neon-content p-8 sm:p-10 flex flex-col items-center relative">
              ...
              <h3 className="text-2xl sm:text-3xl tracking-tighter text-white mb-2 font-black uppercase italic">SYNC RELAY DEVICE</h3>
              ...
              {/* Token TTL Indicator */}
              <div className="mb-5 flex items-center justify-center gap-2 text-[9px] tracking-widest text-[#10B981] font-black uppercase italic">
                <div className="w-2.5 h-2.5 rounded-full bg-[#10B981] animate-pulse"></div>
                <span>SECURE TOKEN ACTIVE — ROTATES EVERY 5 MIN</span>
              </div>
              
              <button onClick={() => { setIsDeviceSynced(true); setShowSyncModal(false); }} className="btn-strategic !bg-[#25F4EE] !text-black text-[10px] sm:text-[11px] w-full py-4 sm:py-5 shadow-xl mb-2 sm:mb-4 font-black uppercase italic">
                CONFIRM DEVICE SYNC
              </button>
            </div>
          </div>
      ...
      {showHelpModal && (
        <div className="fixed inset-0 z-[600] bg-black/95 backdrop-blur-xl flex items-center justify-center p-4 sm:p-6 text-center animate-in fade-in">
          <div className="bg-[#0a0a0a] border border-[#25F4EE]/50 rounded-3xl sm:rounded-[2rem] p-8 sm:p-10 max-w-lg w-full relative shadow-[0_0_50px_rgba(37,244,238,0.2)] max-h-[90vh] overflow-y-auto custom-scrollbar flex flex-col items-center">
            ...
            <div className="bg-[#FE2C55]/10 border border-[#FE2C55]/30 text-[#FE2C55] px-5 sm:px-6 py-4 sm:py-5 rounded-xl flex items-center gap-4 mb-8 text-left w-full uppercase italic">
            ...
            <div className="space-y-4 sm:space-y-5 text-left mb-10 w-full font-black">
              <div className="bg-black border border-white/5 p-5 sm:p-6 rounded-xl sm:rounded-2xl">
                <p className="text-[#25F4EE] font-black text-[10px] sm:text-[11px] tracking-widest mb-2 uppercase italic">STEP 1</p>
              ...
              <div className="bg-black border border-white/5 p-5 sm:p-6 rounded-xl sm:rounded-2xl">
                <p className="text-[#25F4EE] font-black text-[10px] sm:text-[11px] tracking-widest mb-2 uppercase italic">STEP 2</p>
              ...
              <div className="bg-black border border-white/5 p-5 sm:p-6 rounded-xl sm:rounded-2xl">
                <p className="text-[#25F4EE] font-black text-[10px] sm:text-[11px] tracking-widest mb-2 uppercase italic">STEP 3</p>
          ...
          <div className="bg-[#0a0a0a] border border-amber-500/40 rounded-[2rem] w-full max-w-md shadow-[0_0_40px_rgba(245,158,11,0.2)] overflow-hidden">
            <div className="p-6 border-b border-white/10 bg-[#111] flex justify-between items-center">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-sm sm:text-base font-black tracking-widest text-white uppercase italic">EDIT LEAD — MASTER OVERRIDE</h3>
              </div>
              <button onClick={() => setEditLeadModal(null)} className="text-white/30 hover:text-white p-1"><X size={20}/></button>
            </div>
            <div className="p-6 sm:p-8 space-y-5">
              <div>
                <label className="text-[10px] tracking-widest text-white/40 font-black block mb-2 uppercase italic">IDENTITY (NAME)</label>
                <input
              ...
              <div>
                <label className="text-[10px] tracking-widest text-white/40 font-black block mb-2 uppercase italic">TARGET NUMBER (PHONE)</label>
                <input
                  type="tel"
              ...
              <div>
                <label className="text-[10px] tracking-widest text-white/40 font-black block mb-2 uppercase italic">ASSIGN FOLDER / CAMPAIGN</label>
                <select
              ...
              <div className="flex gap-4 pt-3 uppercase italic">
          ...
          <div className="bg-[#0a0a0a] border border-[#25F4EE]/40 rounded-[2rem] w-full max-w-sm shadow-[0_0_40px_rgba(37,244,238,0.15)] overflow-hidden">
            <div className="p-6 border-b border-white/10 bg-[#111] flex justify-between items-center">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-sm sm:text-base font-black tracking-widest text-white uppercase italic">NEW CAMPAIGN FOLDER</h3>
              </div>
              <button onClick={() => setCreateFolderModal(false)} className="text-white/30 hover:text-white p-1"><X size={20}/></button>
            </div>
            <div className="p-6 sm:p-8 space-y-5">
              <div>
                <label className="text-[10px] tracking-widest text-white/40 font-black block mb-2 uppercase italic">FOLDER / CAMPAIGN NAME</label>
                <input
                  value={newFolderName}
